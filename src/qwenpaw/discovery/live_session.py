...
def make_live_session_factory(
    out_root: Path | str | None = None,
) -> Callable[[], LiveDiscoverySession]:
    """Fábrica ``() -> LiveDiscoverySession`` p/ ``set_session_factory``."""

    def _factory() -> LiveDiscoverySession:
        base = Path(out_root) if out_root else None
        session_id = uuid4().hex[:8]
        out_dir = base / session_id if base else None
        return LiveDiscoverySession(session_id=session_id, out_dir=out_dir)

    return _factory


def wire_live_session_if_enabled() -> bool:
    """Liga o ``/discovery`` ao agente real quando ``QWENPAW_DISCOVERY_LIVE``.

    Default seguro: sem a env, mantém a ``ScriptedDiscoverySession``. Devolve
    ``True`` se trocou a fábrica do router para a sessão ao vivo.
    """
    flag = os.environ.get("QWENPAW_DISCOVERY_LIVE", "").strip().lower()
    if flag not in ("1", "true", "yes", "on"):
        return False
    from ..app.routers.discovery_stream import set_session_factory

    set_session_factory(make_live_session_factory())
    return True
