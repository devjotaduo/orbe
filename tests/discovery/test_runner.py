...
class FakeRequirementsAgent:
    ...
    async def reply(self, msg):
        await self.session.emit_requirements(
            json.dumps(
                {
                    "summary_for_owner": (
                        "Olá! Por este grupo vamos pedir o"
                        " que falta e você testa o atendente."
                    ),
                    "items": [
                        {
                            ...
                            "requests": [
                                {
                                    ...
                                    "why": (
                                        "para o atendente responder sozinho"
                                    ),
                                    "group_message": (
...
async def test_runner_scripted_interview(tmp_path, monkeypatch):
    # respostas do empresário. O loop sai quando session.emitted=True
    # após o 2º turno; "/fim" é testado em test_runner_fim_path.
    inputs = iter(
        [
    ...
    # user turns must appear in session.state.transcript
    user_turns = [t for t in out.state.transcript if t.role == "user"]
    assert len(user_turns) >= 2, (
        f"Esperado >= 2 turnos de usuário no transcript, "
        f"encontrado {len(user_turns)}: {out.state.transcript}"
    )
...
async def test_runner_eoferror_persists_and_returns_not_emitted(
    ...
    assert (
        call_count == 1
    ), "EOFError deve ser capturado na primeira chamada de _read_user_input"
    # Não emitiu blueprint (encerrou antes)
    assert out.emitted is False
    # Estado deve ter sido persistido mesmo assim
    assert (tmp_path / "discovery_state.json").exists(), (
        "discovery_state.json deve ser salvo "
        "mesmo quando stdin fecha inesperadamente"
    )
