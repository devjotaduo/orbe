...
def test_cloudpaw_has_ptbr_description():
    p = json.loads(
        (PLUGINS / "cloudpaw/plugin.json").read_text(encoding="utf-8"),
    )
...
def test_qwenpaw_pet_has_ptbr_description():
    p = json.loads(
        (PLUGINS / "qwenpaw-pet/plugin.json").read_text(encoding="utf-8"),
    )
    assert (
        "pt-BR" in p["description_i18n"]
    ), "qwenpaw-pet/plugin.json sem pt-BR"


def test_gpt_image2_has_ptbr_description():
    p = json.loads(
        (TOOL_PLUGINS / "gpt-image2/plugin.json").read_text(encoding="utf-8"),
    )
...
def test_qwen_image_has_ptbr_description():
    p = json.loads(
        (TOOL_PLUGINS / "qwen-image/plugin.json").read_text(encoding="utf-8"),
    )
...
def test_wan27_has_ptbr_description():
    p = json.loads(
        (TOOL_PLUGINS / "wan27/plugin.json").read_text(encoding="utf-8"),
    )
...
def test_qwenpaw_plugin_kit_has_ptbr_description():
    ...
    assert (
        "pt-BR" in p["description_i18n"]
    ), "qwenpaw-plugin-kit/plugin.json sem pt-BR"
